...
\texttt{pymcel} \citep{zuluaga2026pymcel}, con error relativo de energía inferior a $5\times10^{-6}$.
...
ejecutado en Google Colab; el código fuente completo se encuentra disponible en el repositorio de
GitHub del proyecto \citep{Soto_Villada_Modelado_N_Cuerpos}.  Las bibliotecas principales empleadas son:
...
\begin{table}[H]
  ...
  \begin{tabular}{llp{4cm}}
    ...
    \midrule
    \texttt{pymcel} & Integrador N-cuerpos; conversión de elementos orbitales & \citet{zuluaga2026pymcel} \\
...
La función \texttt{pymcel.ncuerpos\_solucion(sistema, ts)} \citep{zuluaga2026pymcel} encapsula
...
\begin{figure}[H]
  ...
  \caption{\textit{Izquierda:} evolución temporal del módulo del momento angular
    específico $h_i(t)$ de las 5 estrellas.  \textit{Derecha:} variación relativa
    $\delta h_i(t) = (h_i(t) - h_i(0))/h_i(0)$.  Las
...
perturbaciones mutuas no modifican sustancialmente el momento angular específico de ninguna de las
cinco estrellas.  La ligera precesión secular del polo orbital es
numéricamente detectable pero físicamente insignificante, como se analiza a continuación.

\paragraph{Análisis analítico del polo orbital.}
El polo orbital de cada estrella, $\hat{\mathbf{h}}_i = \mathbf{h}_i/|\mathbf{h}_i|$, es la normal al plano
orbital.  En el límite de dos cuerpos es exactamente constante; en el sistema de $N+1$ cuerpos las
perturbaciones mutuas inducen una precesión secular.  Siguiendo la teoría de perturbaciones de primer
orden, la fuerza perturbadora media es $F_{\rm pert} \sim N_{\rm est}\,G\langle m_\star\rangle/\langle d\rangle^2$,
donde $\langle d\rangle \sim 2\langle a \rangle$ es la separación característica media entre estrellas.
Tomando la media sobre una órbita y normalizando por la fuerza central
($F_{\SgrA} \sim G M_{\SgrA}/\langle r\rangle^2$, con $\langle r\rangle \approx a$) se obtiene la
razón ya estimada en la ecuación~\eqref{eq:ratio_fuerzas}.  Esta razón de fuerzas, multiplicada
por la frecuencia orbital $2\pi/T_i$, da la tasa de precesión angular del polo orbital:
\begin{equation}
  \dot{\theta}_{\rm prec} \sim
    \frac{N_{\rm est}\,\langle m_\star \rangle}{4\,M_{\SgrA}}
    \cdot \frac{2\pi}{T_i}
  \approx 3.2\times10^{-6} \cdot \frac{2\pi}{T_i}.
  \label{eq:prec_polo}
\end{equation}
Para las cinco estrellas del subconjunto (S1, S2, S8, S9, S12) y un intervalo de
$\Delta t = 100\,\yr$, el desplazamiento angular acumulado es
$\Delta\theta \lesssim 1.3\times10^{-4}\,\text{rad} \approx 0.007^\circ$ en el caso más desfavorable
(S2, período $T \approx 16\,\yr$).  Este valor es \emph{cuatro órdenes de magnitud} menor que la
precesión de Schwarzschild de S2 ($\sim 12'$ por órbita, equivalente a
$\sim 2\times10^{-3}\,\text{rad}\cdot\text{yr}^{-1}$), lo que confirma que las perturbaciones N-cuerpos
no modifican apreciablemente la orientación del plano orbital en la escala temporal analizada.
...
\paragraph{Conservación del momento angular y polo orbital.}
...
de tiempo estudiada.  La tasa de precesión estimada analíticamente mediante la
ecuación~\eqref{eq:prec_polo} es del orden de $10^{-5}$--$10^{-4}\,\text{rad\,yr}^{-1}$,
...
\begin{enumerate}[label=\textbf{C\arabic*.}, itemsep=5pt]
  ...
  \item \textbf{El integrador Adams-Moulton de \texttt{pymcel} es adecuado} \citep{zuluaga2026pymcel} para
